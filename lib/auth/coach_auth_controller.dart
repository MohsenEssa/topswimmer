...
class CoachAuthController extends GetxController {
  static CoachAuthController get to => Get.find();
  late Rx<User?> _user;
  final FirebaseAuth _auth = FirebaseAuth.instance;
  final RxBool isCoach = false.obs;
  ...
  Future<void> addCoachDetails(
    ...
    try {
      DocumentReference coachRef =
          FirebaseFirestore.instance.collection('Coachs').doc(email);

      DocumentSnapshot<Map<String, dynamic>> snapshot =
          await coachRef.get() as DocumentSnapshot<Map<String, dynamic>>;
      if (snapshot.exists) {
  ...
  Future<bool> loginCoach(
    ...
    try {
      ...
      if (user != null) {
        final bool isCoach = await checkCoach(email);
        if (!isCoach) {
          throw Exception("No Coach with these credentials was found!");
        }
      }
      // If checkCoach doesn't throw an exception, the login is successful
      return true;
    } catch (e) {
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: const Text("No Coach with these credentials was found!"),
            actions: [
              TextButton(
                onPressed: () {
  ...
  Future<bool> checkCoach(String? email) async {
    final DocumentSnapshot<Map<String, dynamic>> snapshot =
        await FirebaseFirestore.instance.collection("Coachs").doc(email).get();
  ...
  Future<void> updateCoachDetails(
    ...
    try {
      ...
      DocumentReference CoachRef =
          FirebaseFirestore.instance.collection('Coachs').doc(documentName);
